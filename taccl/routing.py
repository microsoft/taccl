...
class TACCLRouting:
    ...
    def _encode(self, opt):
        ...
        self.send = opt.addVars(C, R, R, L, name="send",
                                vtype=GRB.CONTINUOUS, lb=0.0)
        self.start = opt.addVars(
            C, R, name="start", vtype=GRB.CONTINUOUS, lb=0.0)
        self.time = opt.addVar(name="time", vtype=GRB.CONTINUOUS)
        opt.addLConstr(self.time <= ST-1)
        self.is_sent = opt.addVars(
            C, R, R, L, name="is_sent", vtype=GRB.BINARY)
        if self.route_sketch.intranode.strategy == "minmax":
            self.max_link_util = opt.addVar(
                name="max_link_util", vtype=GRB.CONTINUOUS, lb=0.0)
        if self.route_sketch.intranode.strategy == "maxmin":
            self.min_link_util = opt.addVar(
                name="min_link_util", vtype=GRB.CONTINUOUS, lb=0.0)
        ...
        for c in self.collective.chunks():
            for r in self.collective.ranks():
                ...
                if self.collective.precondition(r, c):
                    ...
                    for src in self.topology.sources(r):
                        for l in range(self.topology.link(src, r)):
                            opt.addGenConstrIndicator(
                                self.is_sent[c, src, r, l], True, self.start[c, r] == self.send[c, src, r, l] + self.latency(src, r, l))
                            opt.addGenConstrIndicator(
                                self.is_sent[c, src, r, l], False, self.send[c, src, r, l] == SND)

                    if self.collective.postcondition(r, c):
                        opt.addLConstr(quicksum(quicksum(self.is_sent[c, src, r, l] for l in range(
                            L)) for src in self.topology.sources(r)) == 1, name=f'post_{r}_{c}')
                        ...
                        opt.addLConstr(quicksum(quicksum(self.is_sent[c, src, r, l] for l in range(
                            L)) for src in range(R)) <= 1, name=f'non_post_{r}_{c}')
                        opt.addLConstr(self.start[c, r] <= self.time + M*(1-quicksum(
                            quicksum(self.is_sent[c, src, r, l] for l in range(L)) for src in range(R))))
                        opt.addLConstr(self.start[c, r] >= self.time + 1 - M*(quicksum(
                            quicksum(self.is_sent[c, src, r, l] for l in range(L)) for src in range(R))))

                for src in self.topology.sources(r):
                    for l in range(self.topology.link(src, r)):
                        opt.addLConstr(
                            self.start[c, src] <= self.send[c, src, r, l])
        ...
        for l, switches in enumerate(self.topology.switches):
            for srcs, dsts, _, swtbw, switch_name in switches:
                if "in" in switch_name:
                    for dst in dsts:
                        opt.addLConstr(self.time >= quicksum(quicksum(
                            swtbw*self.is_sent[c, srci, dst, l] for c in range(C)) for srci in srcs), name=f'switchin_{dst}_{l}')
                        if self.route_sketch.intranode.strategy == "minmax":
                            opt.addLConstr(self.max_link_util >= quicksum(quicksum(
                                swtbw*self.is_sent[c, srci, dst, l] for c in range(C)) for srci in srcs), name=f'Mx_switchin_{dst}_{l}')
                        if self.route_sketch.intranode.strategy == "maxmin":
                            opt.addLConstr(self.min_link_util <= quicksum(quicksum(
                                swtbw*self.is_sent[c, srci, dst, l] for c in range(C)) for srci in srcs), name=f'mx_switchin_{dst}_{l}')
                if "out" in switch_name:
                    for src in srcs:
                        opt.addLConstr(self.time >= quicksum(quicksum(
                            swtbw*self.is_sent[c, src, dsti, l] for c in range(C)) for dsti in dsts), name=f'switchout_{src}_{l}')
                        if self.route_sketch.intranode.strategy == "minmax":
                            opt.addLConstr(self.max_link_util >= quicksum(quicksum(
                                swtbw*self.is_sent[c, src, dsti, l] for c in range(C)) for dsti in dsts), name=f'Mx_switchout_{src}_{l}')
                        if self.route_sketch.intranode.strategy == "maxmin":
                            opt.addLConstr(self.min_link_util <= quicksum(quicksum(
                                swtbw*self.is_sent[c, src, dsti, l] for c in range(C)) for dsti in dsts), name=f'mx_switchout_{src}_{l}')
                        for c in self.collective.chunks():
                            if src in self.spsets[c]:
                                for dstj in dsts:
                                    opt.addGenConstrIndicator(self.is_sent[c, src, dstj, l], True, self.time >= self.start[c, src] + swtbw * quicksum(
        ...
        for r in self.collective.ranks():
            for src in self.topology.sources(r):
                for l in range(self.topology.link(src, r)):
                    opt.addLConstr(self.time >= quicksum(self.latency(
                        src, r, l)*self.is_sent[c, src, r, l] for c in range(C)))
                    if self.route_sketch.intranode.strategy == "minmax":
                        opt.addLConstr(self.max_link_util >= quicksum(self.latency(
                            src, r, l)*self.is_sent[c, src, r, l] for c in range(C)))
                    if self.route_sketch.intranode.strategy == "maxmin":
                        opt.addLConstr(self.min_link_util <= quicksum(self.latency(
                            src, r, l)*self.is_sent[c, src, r, l] for c in range(C)))
        ...
        if isinstance(self.route_sketch.intranode, IntraNode_Switch):
            if self.route_sketch.intranode.switch_hyperedge_strategy[0] == "uc-min":
                ...
                opt.setObjective(self.time + self.mu *
                                 self.unique_links, GRB.MINIMIZE)
            ...
            opt.setObjective(self.time + self.mu *
                             self.max_link_util, GRB.MINIMIZE)
        elif self.route_sketch.intranode.strategy == "maxmin":
            # To do better load balancing
            print("maximizing minimum link utilization")
            opt.setObjective(self.time - self.mu *
                             self.min_link_util, GRB.MINIMIZE)
        else:
    ...
    def _enforce_ordering(self, opt):
        ...
        for sender in sender_to_gpu:
            for cp in range(self.topology.copies):
                ...
                all_chunks = [c for gpu in sender_to_gpu[sender]
                              for c in self.collective.pre_chunk(self.topology.base_gpus[cp] + gpu)]
                        ...
                        for c in all_chunks:
                            if c not in sender_chunks:
                                for c_sender in sender_chunks:
                                    for l in range(self.topology.link(src, r)):
                                        opt.addGenConstrIndicator(
                                            self.is_sent[c_sender, src, r, l], True, self.send[c, src, r, l] >= self.send[c_sender, src, r, l] + self.latency(src, r, l))
    ...
    def _add_symmetry(self, opt, L):
        ...
        for (sym_offset, sym_size) in self.route_sketch.symmetry.offsets:
            already_added = []
            for c in self.collective.chunks():
                c_sym, r_pre_sym, r_post_sym = self.sym_chunk(
                    c, 1, sym_offset, sym_size)
                if c_sym == -1:
                ...
                for r in range(num_nodes):
                    ...
                    for src in range(num_nodes):
                        if (r // sym_size == src // sym_size):
                            src_sym = self.sym_rank(
                                src, 1, sym_offset, sym_size)
                            for l in range(L):
                                opt.addLConstr(
                                    self.send[c, src, r, l] == self.send[c_sym, src_sym, r_sym, l])
                                opt.addLConstr(self.is_sent[c, src, r, l] == self.is_sent[c_sym,
                                               src_sym, r_sym, l], name=f'sym_{c}_{src}_{r}_{src_sym}_{r_sym}_{l}')
                    opt.addLConstr(self.start[c, r] ==
                                   self.start[c_sym, r_sym])
        ...
        for (strategy, nnodes, group_size) in zip(self.route_sketch.multinode.strategy, self.route_sketch.multinode.nnodes, self.route_sketch.multinode.group_size):
            if strategy == "round-robin" or strategy == "relay":
                ...
                for base_n in range(0, group_size*num_groups, nnodes):
                    ...
                    all_gpus[base_n] = [g for g in range(
                        self.topology.base_gpus[base_n], self.topology.base_gpus[base_n + nnodes])]
                for c in self.collective.chunks():
                        ...
                        for r2 in self.collective.post_rank(c):
                            ...
                            if (base_n1 != base_n2) and (n1 // group_size == n2 // group_size):
                                ...
                                if self.route_sketch.internode.gpus_to_sender_rev_map is not None:
                                    ...
                                    assert len(set(chunk_to_sender_map[c]) & set(
                                        senders)) == len(set(chunk_to_sender_map[c]))
                                    senders = [
                                        g for g in chunk_to_sender_map[c]]
                                # remove senders and receivers that are not in spsets
                                senders = list(
                                    filter(lambda x: x in self.spsets[c], senders))
                                receivers = list(
                                    filter(lambda x: x in self.spsets[c], receivers))
                                for s in senders:
                                    for r in receivers:
                                        if self.topology.link(s, r) > 0:
                                            pair_set[(base_n1, base_n2)].add(
                                                (s, r))
                    # print("pair set", c, pair_set)
                    for (bn1, bn2) in pair_set:
                        opt.addLConstr(quicksum(self.is_sent[c, src, r, l] for (src, r) in pair_set[(
                            bn1, bn2)] for l in range(self.topology.link(src, r))) >= 1)
                        for src in all_gpus[bn1]:
                            for r in all_gpus[bn2]:
                                if (src, r) not in pair_set[(bn1, bn2)]:
                                    for l in range(self.topology.link(src, r)):
                                        opt.addLConstr(
                                            self.send[c, src, r, l] == SND)
                                        opt.addLConstr(
                                            self.is_sent[c, src, r, l] == 0, name=f'relay_notSend_{c}_{src}_{r}_{l}')
                                assert (r, src) not in pair_set[(bn1, bn2)]
                                for l in range(self.topology.link(r, src)):
                                    opt.addLConstr(
                                        self.send[c, r, src, l] == SND)
                                    opt.addLConstr(
                                        self.is_sent[c, r, src, l] == 0, name=f'relay_notSend_{c}_{r}_{src}_{l}')
                                    ...
                                    if (src not in all_gpus[base_n1]) or (r not in all_gpus[base_n1]):
                                        for l in range(self.topology.link(src, r)):
                                            opt.addLConstr(
                                                self.send[c, src, r, l] == SND)
                                            opt.addLConstr(
                                                self.is_sent[c, src, r, l] == 0, name=f'relay_sendNotNeeded_{c}_{src}_{r}_{l}')
    ...
    def _add_min_max_unique(self, opt, num_local_nodes, mu, L):
        ...
        # print("SEND_AT_ALL")
        self.send_at_all = opt.addVars(
            num_local_nodes, num_local_nodes, L, name="send_at_all", vtype=GRB.BINARY)
            ...
            for src in self.topology.sources(r):
                if src < num_local_nodes:
                    for l in range(L):
                        for c in self.collective.chunks():
                            opt.addLConstr(
                                self.send_at_all[src, r, l] >= self.is_sent[c, src, r, l])
                        opt.addLConstr(self.send_at_all[src, r, l] <= quicksum(
                            self.is_sent[c, src, r, l] for c in self.collective.chunks()))
        # print("mu", mu)
        self.mu = mu
        self.unique_links = quicksum(self.send_at_all[src, r, l] for l in range(L) for r in range(
            num_local_nodes) for src in self.topology.sources(r) if src < num_local_nodes)
    ...
    def optimize(self, distribute_over_links):
        ...
        instance_name = 'sccl_{}_{}_gurobiSimple'.format(
            self.topology.name, self.collective.name)
        ...
        for sol_i in range(num_sols):
            ...
            time_recv = [[[[] for l in range(L)]
                          for src in range(R)] for r in range(R)]
            chunk_recv = [[[[] for l in range(L)]
                           for src in range(R)] for r in range(R)]
            time_send = [[[[] for l in range(L)]
                          for src in range(R)] for r in range(R)]
            chunk_send = [[[[] for l in range(L)]
                           for src in range(R)] for r in range(R)]
            ...
            # Distribute chunk sends if there are multiple links connecting src to r
            dist_link_heuristic = [3, 5, 8, 9, 10, 11, 13]
                    ...
                    for src in self.topology.sources(r):
                        for l in range(L):
                            if self.is_sent[c, src, r, l].Xn >= 0.995:
                                ...
                                sratch_str[t_val].append(
                                    f'{c}: {src} --{l}--> {r}  t={self.send[c,src,r,l].Xn}\n')
                                # model_str += f'{c}: {src} --{l}--> {r}  t={self.send[c,src,r,l].Xn}\n'
                                if distribute_over_links:
                                    chunk_send[src][r][0].append(c)
                                    time_send[src][r][0].append(
                                        int(self.send[c, src, r, l].Xn + 0.005))
                                    chunk_recv[r][src][0].append(c)
                                    time_recv[r][src][0].append(
                                        int(self.start[c, r].Xn + 0.005))
                                else:
                                    chunk_send[src][r][l].append(c)
                                    time_send[src][r][l].append(
                                        int(self.send[c, src, r, l].Xn + 0.005))
                                    chunk_recv[r][src][l].append(c)
                                    time_recv[r][src][l].append(
                                        int(self.start[c, r].Xn + 0.005))
    ...
    def check_heuristic(self, topology, route_sketch, collective, ts_heur):
        ...
        print(
            f"Checking sol obtained by heuristic {route_sketch.hyperparameters.heuristic} ts={ts_heur}")
